...
function volumeCylinder(radius,height){
    let res=radius*radius*Math.PI*height;
    return res;
}
...
function divGenerator(text){
    document.write(`<div>Your text is here: ${text}</div>`);
}
divGenerator(lorem);

// - створити функцію яка створює ul з трьома елементами li. Текст li задати через аргумент всім однаковий
function listGenerator(text){
    document.write(`<ul>`);
...
function bigListGenerator(text, count){
    document.write(`<ul>`);
...
function listArrayGenerator(array){
    document.write(`<ul>`);
    for (const arrayElement of array) {
...
function displayArrayHTML(array){
    for (const arrayElement of array) {
